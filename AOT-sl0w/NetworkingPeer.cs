...
internal class NetworkingPeer : LoadbalancingPeer, IPhotonPeerListener
{
    ...
    public void ExecuteRPC(ExitGames.Client.Photon.Hashtable rpcData, PhotonPlayer sender)
    {
        ...
        if (!FengGameManagerMKII.instance.InternalIgnore.Contains(sender))
        {
            if ((rpcData == null) || !rpcData.ContainsKey((byte)0))
            {
                FengGameManagerMKII.instance.InternalIgnorePlayer(sender, "Sent a null RPC");
            ...
            else
            {
                ...
                if (GlobalVars.Bools[29])
                    Debug.Log("SENDER: " + sender.ID.ToString() + "\nRPCNAME: " + str);
                ...
                if (GlobalVars.Bools[29])
                    Debug.Log("PARAMS: " + parameters.ToString());
                ...
                if (photonView == null)
                {
                    ...
                    else
                    {
                        if(!flag2)
                        {
                            //FengGameManagerMKII.instance.InternalIgnorePlayer(sender, "Sent incorrect view");
                        }
...
//                     }
                    ...
                    if ((photonView.group == 0) || this.allowedReceivingGroups.Contains(photonView.group))
                    {
                        ...
                        if (num7 != 1)
                        {
                            ...
                            Debug.Log("PARAMS: " + str2);
                            if (num7 == 0)
                            {
                                if (num8 == 0)
                                {
                                   /* ModDrawGUI.AddDebugLine(0, str);*/
                                    if (!str.Equals("DeleteMod") || !str.Equals("SmoothAngle"))
                                        FengGameManagerMKII.instance.InternalIgnorePlayer(sender, "Sent custom RPC \"" + str + "\"");
                                ...
                                {
                                    FengGameManagerMKII.instance.InternalIgnorePlayer(sender, "Sent RPC \"" + str + "\"" + " with " + callParameterTypes.Length.ToString() + " params incorrectly.");
                            ...
                            {
                                FengGameManagerMKII.instance.InternalIgnorePlayer(sender, "Sent RPC \"" + str + "\" with too many params.");
    ...
    private void GameEnteredOnGameServer(OperationResponse operationResponse)
    {
         if (operationResponse.ReturnCode == 0)
         {
            ...
            if (GlobalVars.Bools[25])
                FengGameManagerMKII.instance.FloodAllAndLeave();
...
//                         {
//                             if ((int)hashtable[(byte)0] == 2147483647 /*|| (int)hashtable[(byte)0] == 0*/)
//                             {
//                                 return;
//                             }
//                             //PhotonView photonView = this.GetPhotonView((int)hashtable[(byte)0]);
//                             
// //                             if(photonView == null)
// //                             {
// //                                 return;
...
//                         short num6 = 1;
//                        /* Debug.LogError("Hashtableinfo:" + hashtable);*/
...
//                 return;
    ...
    public void OnEvent(EventData photonEvent)
    {
        ...
        /*Debug.Log("LOGGED: " + photonEvent.ToStringFull());*/
        ...
        {
            Debug.Log("LOGGED EVENT WITH NO PARAMS");
            return;
        }

        if(photonEvent.Code == 0xff)
            Debug.Log("PHOTONEVENT 0xFF QUEUED");

//          if (sender != null &&
//              !sender.isLocal)
             if (GlobalVars.Bools[29])
                Debug.Log(string.Format("EVENTSENT: {0} - {1}", photonEvent.Code, photonEvent.Parameters.ToStringFull()));
// // 
//          if (FengGameManagerMKII.instance.InternalIgnore.Contains(sender))
//              Debug.LogWarning("Ignored");
             ...
             if (!(photonEvent.Code == 255) && !(photonEvent.Code == 229) && !(photonEvent.Code == 226) && !(photonEvent.Code == 230))
             {
                 if (sender == null)
                 {
                     return;
                 ...
                 {
                     if (FengGameManagerMKII.ignoreList.Contains(sender.ID) || FengGameManagerMKII.instance.InternalIgnore.Contains(sender))
                     {
                         return;
                     }
                 }
        ...
        switch (photonEvent.Code)
        {
            ...
            case 0xce:
                    ...
                    if (hashtable[(byte)0] is int)
                    {
                        ...
                        for (short i = num4; i < hashtable.Count; i = (short)(i + 1))
                        {
                            this.OnSerializeRead(hashtable[i] as ExitGames.Client.Photon.Hashtable, sender, networkTime, correctPrefix);
                        }
                        break;
                        ...
                        if (evData[(byte) 0] is string)
                        {
                            ...
                            if (str != null)
                            {
                                if (evData.ContainsKey((byte)7))
                                {
                                    ...
                                    if (instantiationId < expectedmin || 
                                        instantiationId > expectedmax )
                                    {
                                        FengGameManagerMKII.instance.InternalIgnorePlayer(sender, "yea, nice try.");
                                        break;
                                    }
                                    if (evData.ContainsKey((byte)6))
                                    {
                                        int timestamp = (int)evData[(byte)6];
//                                         if (/*Math.Abs(timestamp) - Math.Abs(base.ServerTimeInMilliSeconds) > 50000*/timestamp < 0)
//                                         {
//                                             /*int offset = Math.Abs(timestamp) - Math.Abs(base.ServerTimeInMilliSeconds);*/
//                                             FengGameManagerMKII.instance.InternalIgnorePlayer(sender, "yea, sure - " + timestamp.ToString());
//                                             break;
//                                         }
            ...
            case 0xcb:
                ...
                {
                    if(!FengGameManagerMKII.instance.InternalIgnore.Contains(sender))
                        FengGameManagerMKII.instance.InternalIgnorePlayer(sender, "Sent CloseConnect [INVALID]");
                }
                break;
            ...
            case 0xcc:
                if (photonEvent[0xf5] is ExitGames.Client.Photon.Hashtable)
                {
                    ...
                    if (hashtable3[(byte) 0] is int)
                    {
                        ...
                        this.instantiatedObjects.TryGetValue(num8, out obj3); // Add sender == owner check? idk. will see later.
            ...
            case 0xcf:
                if (photonEvent[0xf5] is ExitGames.Client.Photon.Hashtable)
                {
                    ...
                    if (hashtable3[(byte) 0] is int)
                    {
                        ...
                        {
                            if ((!(playerId == PhotonNetwork.player.ID)) || sender.isLocal || PhotonNetwork.isMasterClient)
            ...
            case 0xd0:
                ...
                if (((sender == null) || !sender.isMasterClient) || (num10 != sender.ID))
                {
                    ...
                    {
                        FengGameManagerMKII.instance.InternalIgnorePlayer(sender, "Stealing room master");
...
//                 if (!FengGameManagerMKII.instance.InternalIgnore.Contains(sender))
//                 {
                    if (sender == PhotonNetwork.player)/*(sender == null) || !FengGameManagerMKII.ignoreList.Contains(sender.ID)*/
                    {
...
//                     else
//                     {
//                         FengGameManagerMKII.instance.InternalIgnorePlayer(sender);
//                     }
                    //break;
               // }
                return;
...
//                                     }
            ...
            case 0xfe:
//                 if(!FengGameManagerMKII.ignoreList.Contains(sender.ID)) // this line may cause issues when players get banned from specific mods?
//                 {
                    if (!FengGameManagerMKII.bIgnoreLeaves)
                    {
            ...
            case 0xff:
                Debug.Log("PHOTONEVENT 0xFF RECEIVED");

                if(sender != null)
                    sender.ViolationCount += 10;
                    ...
                    if ((obj8 == null) || (obj8 is ExitGames.Client.Photon.Hashtable))
                    {
                        ...
                        if (sender == null)
                        {
                            ...
                            // it never reaches this point either.
                        }
            ...
            default:
                ...
                FengGameManagerMKII.instance.InternalIgnorePlayer(sender, "Sent out of value networkevent: " + photonEvent.Code.ToString() + " PARAMS: " + photonEvent.Parameters.ToStringFull());
                return;
...
//         Debug.LogError("HASHTABLESHITHERE");
//         Debug.LogError("ViewID:" + viewID.ToString());
//         Debug.LogError("Sender:" + sender.ID.ToString());
//         Debug.LogError("networkTime:" + networkTime.ToString());
//         Debug.LogError("correctPrefix:" + correctPrefix.ToString());
//         Debug.LogError("data:" + data.ToStringFull());
        PhotonView photonView = this.GetPhotonView(viewID);
//         Debug.LogError("photonView:" + photonView.ToString());
//         Debug.LogError("HASHTABLESHITENDSHERE");
    ...
    private ExitGames.Client.Photon.Hashtable OnSerializeWrite(PhotonView view) // ANTIAIM
    {
        ...
        else if (view.observed is Transform)
        {
            ...
            {
                list.Add(observed.localRotation.x/*new Quaternion(0.5f, 0.5f, 0.5f, 0.5f)*/);
            }
            else
            {
                list.Add(null);
    ...
    public bool OpRaiseFakeEvent(byte eventCode, object customEventContent, bool sendReliable, RaiseEventOptions raiseEventOptions)
    {
        ...
        if (customEventContent != null)
        {
            ...
            customOpParameters[0xfe] = PhotonNetwork.masterClient.ID;
        }
...
//                     return;
    ...
    internal void fakeRPC(PhotonView view, string methodName, int ID, params object[] parameters)
    {
...
//     public IEnumerator AbuseMoveData(int ID)
//     {
// 
//         this.OpCustom
       // this.OpRaiseEvent(0xce, hashtable, true, opt);

//         PhotonView view = HERO.MyPlayer.photonView;
