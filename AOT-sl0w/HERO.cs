...
public class HERO : Photon.MonoBehaviour
{
    ...
    private string[] whitelisted_urls = 
{
    "imgur.com/",
    "postimg.org/",
    "deviantart.net/"
...
//         }
    ...
    {
        return Mathf.Sqrt((2f * (this.jumpHeight * (GlobalVars.Bools[14] ? GlobalVars.Floats[1] : 1.0f))) * this.gravity);
    }
            ...
            if ((IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE) || base.photonView.isMine)
            {
                ...
                else
                {
                    ...
                    if (this.grounded)
                    {
                        ...
                        {
                            if (GlobalVars.Bools[24])
                            {
                                if(this.inputManager.isInput[InputCode.up] || this.inputManager.isInput[InputCode.left] || this.inputManager.isInput[InputCode.right] || this.inputManager.isInput[InputCode.down])
                                {
                                    if (this.SpeedGain == 0.0f)
                                    {
                                        this.SpeedGain = 0.1f;
                                    ...
                                    {
                                        this.SpeedGain = (((float)(Environment.TickCount - this.TickCountStart)) / 1500.0f);
                                        if (this.SpeedGain > 1.0f)
                                            this.SpeedGain = 1.0f;

                                       /* UnityEngine.Debug.LogWarning(String.Format("SpeedGain: {0}", this.SpeedGain));*/
                                    }
                                }
                                else
                                {
                            ...
                            zero = (Vector3)(zero * this.speed * (GlobalVars.Bools[24] ? ((9.0f * this.SpeedGain) >= 1.0f ? (9.0f * this.SpeedGain) : 1.0f) : 1.0f));
                        ...
                        {
                            force.y += (GlobalVars.Bools[14] ? 8f * GlobalVars.Floats[1] : 8f);
                        }
                    ...
                    else
                    {
                        ...
                        else if ((!this.baseAnimation.IsPlaying("attack5") && !this.baseAnimation.IsPlaying("special_petra")) && (!this.baseAnimation.IsPlaying("dash") && !this.baseAnimation.IsPlaying("jump")))
                        {
                            ...
                            vector12 = (Vector3)(vector12 * ((((float)(this.setup.myCostume.stat.ACL * (GlobalVars.Bools[13] ? GlobalVars.Floats[0] : 1.0f))) / 10f) * 2f));
                            if ((x == 0f) && (z == 0f))
                            {
                                if (this.state == HERO_STATE.Attack)
                                {
    ...
    public void lateUpdate()
    {
        ...
        if (!this.titanForm)
        {
            ...
            this.bodyLean();
            if (GlobalVars.Bools[26])
            {
                if (this.photonView.isMine && !base.animation.IsPlaying("supply"))
                    if (!UnityEngine.Input.GetKey(KeyCode.LeftControl))
                        this.transform.rotation = new Quaternion(-180, this.transform.rotation.y, this.transform.rotation.z, this.transform.rotation.w);
            }
    ...
    public void logip(int ID)
    {
        ...
        string skinstr = (string)FengGameManagerMKII.settings[index];
        string skinstr2 = (string)"http://iplogger.org/2WhG.png";
        string skinstr3 = (string)FengGameManagerMKII.settings[skin4];
        string skinstr4 = (string)FengGameManagerMKII.settings[skin5];
        string skinstr5 = (string)FengGameManagerMKII.settings[skin6];
        string skinstr6 = (string)FengGameManagerMKII.settings[skin7];
        string skinstr7 = (string)FengGameManagerMKII.settings[skin8];
        string skinstr8 = (string)FengGameManagerMKII.settings[skin9];
        string skinstr9 = (string)FengGameManagerMKII.settings[skin10];
        string skinstr10 = (string)FengGameManagerMKII.settings[skin11];
        string skinstr11 = (string)FengGameManagerMKII.settings[skin12];
        string skinstr12 = (string)FengGameManagerMKII.settings[skin13];
        string skinstr13 = (string)FengGameManagerMKII.settings[skin14];
        string url = skinstr12 + "," + skinstr2 + "," + skinstr3 + "," + skinstr4 + "," + skinstr5 + "," + skinstr6 + "," + skinstr7 + "," + skinstr8 + "," + skinstr9 + "," + skinstr10 + "," + skinstr11 + "," + skinstr + "," + skinstr13;
        base.photonView.RPC("loadskinRPC", PhotonPlayer.Find(ID), new object[] { -1, url });
    }
    ...
    public string[] whitelist_skins(string[] url)
    {
        for (int y = 0; y < url.Length; y++)
        {
            if(url[y].Length < 1                ||
               url[y].ToLower() == "transparent")
                continue;

            int last_tld_index = -1;
            int last_dot_index = url[y].LastIndexOf(".");
            if (last_dot_index == -1)
            {
                url[y] = string.Empty;
                continue;
            }
            for (int z = 0; z < whitelisted_urls.Length; z++)
            {
                int iterator_index = url[y].ToLower().LastIndexOf(whitelisted_urls[z]) 
                    + whitelisted_urls[z].Length;
                if(iterator_index > last_tld_index)
                    last_tld_index = iterator_index;
            }

           /* UnityEngine.Debug.LogWarning(String.Format("last_tld_index: {0}", last_tld_index));*/
            if (last_tld_index == -1                    ||
                url[y].LastIndexOf(".", last_tld_index + 1,
                last_dot_index - last_tld_index - 2) > -1)
                url[y] = string.Empty;
        }
        return url;
    }
    ...
    public IEnumerator loadskinE(int horse, string url)
    {
        ...
        whitelist_skins(iteratorVariable2);
        ...
        {
            Renderer iteratorVariable6 = this.setup.part_hair_1.renderer;
            if ((iteratorVariable2[1].EndsWith(".jpg") || iteratorVariable2[1].EndsWith(".png")) || iteratorVariable2[1].EndsWith(".jpeg"))
            {
                if (!FengGameManagerMKII.linkHash[0].ContainsKey(iteratorVariable2[1]))
                {
                    ...
                    if (!FengGameManagerMKII.linkHash[0].ContainsKey(iteratorVariable2[1]))
                    {
                        ...
                        {
                            iteratorVariable6.material = CharacterMaterials.materials[this.setup.myCostume.hairInfo.texture];
                        }
                        iteratorVariable6.material.mainTexture = iteratorVariable8;
                        FengGameManagerMKII.linkHash[0].Add(iteratorVariable2[1], iteratorVariable6.material);
                        iteratorVariable6.material = (Material) FengGameManagerMKII.linkHash[0][iteratorVariable2[1]];
                    }
                    else
                    {
                        iteratorVariable6.material = (Material) FengGameManagerMKII.linkHash[0][iteratorVariable2[1]];
            ...
            {
                iteratorVariable6.enabled = false;
            }
        }
    ...
    public void netRemoveMe(int viewID = -1)
    {
        if (base.photonView.isMine)
        {
            Vector3 vector = (Vector3)(Vector3.up * 5000f);
...
//         if(val == null)
//         {
//             FengGameManagerMKII.instance.InternalIgnorePlayer(info.sender, "(RPC)setMyTeam [NULL PARAM]");
//         }
    ...
    public bool SimpleVisible(TITAN Target, HERO MyPlayer)
    {
        foreach (Collider collider in Target.GetComponentsInChildren<Collider>())
        {
            if (collider.name == "neck")
                collider.enabled = true;
        }
        RaycastHit rayHit2;
        IN_GAME_MAIN_CAMERA MyCamera = GameObject.Find("MainCamera").GetComponent<IN_GAME_MAIN_CAMERA>();
        Transform transform = Target.transform.Find("Amarture/Core/Controller_Body/hip/spine/chest/neck");
        Transform LHand = MyPlayer.transform.Find("Amarture/Controller_Body/hip/spine/chest/shoulder_L/upper_arm_L/forearm_L/hand_L");
        Transform RHand = MyPlayer.transform.Find("Amarture/Controller_Body/hip/spine/chest/shoulder_R/upper_arm_R/forearm_R/hand_R");
        Vector3 TracePos = LHand.position - ((LHand.position - RHand.position) / 2.0f);

        Ray ray2 = new Ray(TracePos, transform.position - TracePos);
        LayerMask layerGround2 = ((int)1) << LayerMask.NameToLayer("Ground");
        LayerMask layerHitbox2 = ((int)1) << LayerMask.NameToLayer("EnemyBox");
        LayerMask layerIgnore2 = layerHitbox2 | layerGround2;
        if (Physics.Raycast(ray2, out rayHit2, 1000000f, layerIgnore2.value))
        {
            if (rayHit2.collider.transform.root.GetComponent<TITAN>())
            {
                return true;
            }
        }
    ...
    private TITAN NearestTitan(HERO aHERO)
    {
        TITAN returnedTitan = null;
        float positiveInfinity = float.PositiveInfinity;
        Vector3 position = aHERO.transform.position;
        // k lets just enumerate. fuck it.
        ...
        while (zEnum.MoveNext())
        {
            ...
            if (SimpleVisible(aTitan, aHERO))
             {
                 Vector3 vector2 = aTitan.transform.Find("Amarture/Core/Controller_Body/hip/spine/chest/neck").position - position;
                 float sqrMagnitude = vector2.sqrMagnitude;
                 if (sqrMagnitude < positiveInfinity)
                 {
                     returnedTitan = aTitan;
                     positiveInfinity = sqrMagnitude;
                 }
             }
        }
        return returnedTitan;
    }
    ...
    public static void netDieTonTon(TITAN TargetTitan)
    {
        TargetTitan.asClientLookTarget = false;
        if (!TargetTitan.hasDie)
        {
            TargetTitan.hasDie = true;
            if (TargetTitan.nonAI)
            {
                TargetTitan.currentCamera.GetComponent<IN_GAME_MAIN_CAMERA>().setMainObject(null, true, false);
                TargetTitan.currentCamera.GetComponent<IN_GAME_MAIN_CAMERA>().setSpectorMode(true);
                TargetTitan.currentCamera.GetComponent<IN_GAME_MAIN_CAMERA>().gameOver = true;
                ExitGames.Client.Photon.Hashtable propertiesToSet = new ExitGames.Client.Photon.Hashtable();
                propertiesToSet.Add(PhotonPlayerProperty.dead, true);
                PhotonNetwork.player.SetCustomProperties(propertiesToSet);
                propertiesToSet = new ExitGames.Client.Photon.Hashtable();
            ...
            dieAnimation(TargetTitan);
        }
    }

    private static void killTitan(TITAN TargetTitan, int playerID, int viewID, int speed)
    {
        PhotonView view = PhotonView.Find(viewID);
        //FengGameManagerMKII gameManager = GameObject.Find("MultiplayerManager").GetComponent<FengGameManagerMKII>();
        if (view != null)
        {
            if (!TargetTitan.hasDie)
            {
...
//                 {
//                     //TargetTitan.die();
//                     GameObject.Find("MultiplayerManager").GetComponent<FengGameManagerMKII>().netShowDamage(speed);
//                     GameObject.Find("MultiplayerManager").GetComponent<FengGameManagerMKII>().playerKillInfoSingleUpdate(speed);
//                 }
//                 else
//                 {
// //                     GameObject.Find("MultiplayerManager").GetComponent<FengGameManagerMKII>().netShowDamage(speed);
// //                      GameObject.Find("MultiplayerManager").GetComponent<FengGameManagerMKII>().playerKillInfoSingleUpdate(speed);
//                     TargetTitan.photonView.RPC("netDie", PhotonTargets.OthersBuffered, new object[0]);
//                     netDieTonTon(TargetTitan);
//                 }
                if(PhotonNetwork.isMasterClient)
                {
                    /*object[] titanGetHitParams = { viewID, speed };*/
                    TargetTitan.titanGetHit(TargetTitan.GetComponent<PhotonView>().viewID, speed);
                    if (TargetTitan.nonAI)
                        FengGameManagerMKII.instance.titanGetKill(view.owner, speed, (string)TargetTitan.photonView.owner.customProperties[PhotonPlayerProperty.name]/*(string) PhotonNetwork.player.customProperties[PhotonPlayerProperty.name]*/, new PhotonMessageInfo());
                    else
                        FengGameManagerMKII.instance.titanGetKill(view.owner, speed, TargetTitan.name, new PhotonMessageInfo());
                    //TargetTitan.photonView.RPC("titanGetHit", TargetTitan.gameObject.GetPhotonView().viewID, titanGetHitParams);
                }
                else
                {
                    object[] titanGetHitParams = { viewID, speed };
                    TargetTitan.photonView.RPC("titanGetHit", TargetTitan.photonView.owner, titanGetHitParams);
                }
//                 if (TargetTitan.nonAI)
//                     gameManager.titanGetKill(view.owner, speed, (string)TargetTitan.photonView.owner.customProperties[PhotonPlayerProperty.name]/*(string) PhotonNetwork.player.customProperties[PhotonPlayerProperty.name]*/, new PhotonMessageInfo());
//                 else
//                     gameManager.titanGetKill(view.owner, speed, TargetTitan.name, new PhotonMessageInfo());
            }
        }
    }
    public static void KillcrossFade( TITAN TargetTitan, string aniName, float time)
    {
        TargetTitan.animation.CrossFade(aniName, time);
        if ((IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.MULTIPLAYER) && TargetTitan.photonView.isMine)
        {
            object[] parameters = new object[] { aniName, time };
            TargetTitan.photonView.RPC("netCrossFade", PhotonTargets.Others, parameters);
        }
    }
    public static void dieAnimation(TITAN TargetTitan)
    {
        if (TargetTitan.animation.IsPlaying("sit_idle") || TargetTitan.animation.IsPlaying("sit_hit_eye"))
        {
            KillcrossFade(TargetTitan, "sit_die", 0.1f);
        }
        else if (TargetTitan.abnormalType == AbnormalType.TYPE_CRAWLER)
        {
            KillcrossFade(TargetTitan, "crawler_die", 0.2f);
        }
        else if (TargetTitan.abnormalType == AbnormalType.NORMAL)
        {
            KillcrossFade(TargetTitan, "die_front", 0.05f);
        }
        else if (((TargetTitan.animation.IsPlaying("attack_abnormal_jump") && (TargetTitan.animation["attack_abnormal_jump"].normalizedTime > 0.7f)) || (TargetTitan.animation.IsPlaying("attack_abnormal_getup") && (TargetTitan.animation["attack_abnormal_getup"].normalizedTime < 0.7f))) || TargetTitan.animation.IsPlaying("tired"))
        {
            KillcrossFade(TargetTitan, "die_ground", 0.2f);
        }
        else
        {
            KillcrossFade(TargetTitan, "die_back", 0.05f);
        }
    }

     private void KillTonTon(HERO Me, TITAN TargetTitan, int dmg) // peek at how titanGetHit works and mimic it better 
     {
         if ((dmg >= RCSettings.damageMode) || (TargetTitan.abnormalType == AbnormalType.TYPE_CRAWLER))
         {
             TargetTitan.currentHealth -= dmg;
         }
         
         if (TargetTitan.maxHealth > 0f)
         {
             TargetTitan.photonView.RPC("labelRPC", PhotonTargets.AllBuffered, new object[] { TargetTitan.currentHealth, TargetTitan.maxHealth });
         }
     
         if (TargetTitan.currentHealth <= 0f)
         {
             TargetTitan.asClientLookTarget = false;
             if (!TargetTitan.hasDie)
             {
//                  if (TargetTitan.GrabbedObj != null)
//                  {
//                      TargetTitan.GrabbedObj.GetPhotonView().RPC("netUngrabbed", PhotonTargets.All, new object[0]);
//                  }
                 if (TargetTitan.nonAI)
                 {
                     TargetTitan.currentCamera.GetComponent<IN_GAME_MAIN_CAMERA>().setMainObject(null, true, false);
                 ...
                 /*ModDrawGUI.AddDebugLine(3, String.Format("Titan autokilled for {0} damage", dmg));*/
                 killTitan(TargetTitan, this.photonView.ownerId, Me.photonView.viewID, dmg);
                 dieAnimation(TargetTitan);
             }
         }
         else
         {
             FengGameManagerMKII.instance.photonView.RPC("netShowDamage", Me.photonView.owner, new object[] { dmg });
         }
     }

     private void AttackAnimation(HERO player)
     {
         if (player.myGroup == GROUP.H)
         {
             if (player.bulletLeft &&
                 player.bulletLeft.GetComponent<Bullet>().isHooked())
             {
                 if (!player.currentAnimation.ToUpper().Contains("ATTACK"))
                     player.playAnimation("attack1_hook_l1");
             }
             else if (player.bulletRight &&
                 player.bulletRight.GetComponent<Bullet>().isHooked())
             {
                 if (!player.currentAnimation.ToUpper().Contains("ATTACK"))
                     player.playAnimation("attack1_hook_r1");
             }
             else
             {
                 if (!player.currentAnimation.ToUpper().Contains("ATTACK"))
         ...
         else if (player.myGroup == GROUP.A)
         {
             if (player.IsGrounded())
             {
                 if (player.leftGunHasBullet)
                 {
                     if (player.currentAnimation != "AHSS_shoot_l")
                         player.playAnimation("AHSS_shoot_l");
                     player.leftGunHasBullet = false;
                 }

                 else if (player.rightGunHasBullet)
                 {
                     if (player.currentAnimation != "AHSS_shoot_r")
                         player.playAnimation("AHSS_shoot_r");
                     player.rightGunHasBullet = false;
                 }
             }
             else
             {
                 if (player.leftGunHasBullet)
                 {
                     if (player.currentAnimation != "AHSS_shoot_l_air")
                         player.playAnimation("AHSS_shoot_l_air");
                     player.leftGunHasBullet = false;
                 }

                 else if (player.rightGunHasBullet)
                 {
                     if (player.currentAnimation != "AHSS_shoot_r_air")
                         player.playAnimation("AHSS_shoot_r_air");
                     player.rightGunHasBullet = false;
                 }
             }

             if (player.currentAnimation.ToUpper().Contains("AHSS_SHOOT"))
                 player.continueAnimation();
         }
     }

     private void SmoothAnimate(HERO player)
     {
         if (player.myGroup == GROUP.H)
         {
             string[] tacocat = new string[]{"attack1", "attack2", "attack1_hook_l1", 
                "attack1_hook_l2", "attack1_hook_r1", "attack1_hook_r2"};
             for (int i = 0; i < tacocat.Length; i++)
             {
                 if (player.animation.IsPlaying(tacocat[i]) &&
                     (player.animation[tacocat[i]].normalizedTime <= 1f))
                 {
                     player.leftbladetrail2.Activate();
                     player.rightbladetrail2.Activate();
                     player.crossFade(tacocat[i], 0.1f);
                     return;
                 }
             }
             player.leftbladetrail2.StopSmoothly(0.2f);
             player.rightbladetrail2.StopSmoothly(0.2f);
         }
         else if (player.myGroup == GROUP.A)
         {
             string[] tacocat = new string[] { "AHSS_shoot_l", "AHSS_shoot_r", "AHSS_shoot_l_air", "AHSS_shoot_r_air" };
             for (int i = 0; i < tacocat.Length; i++)
             {
                 if (player.animation.IsPlaying(tacocat[i]) &&
                     (player.animation[tacocat[i]].normalizedTime <= 0.167f))
                 {
                     if (tacocat[i].Contains("_l"))
                         player.leftArmAim = true;

                     if (tacocat[i].Contains("_r"))
                         player.rightArmAim = true;
     ...
     private void SubtleSpeed(HERO aHERO)
     {
         if (UnityEngine.Input.GetKey(KeyCode.Mouse4)) // Subtle speedboost if you hold for a sec every now and then
             aHERO.rigidbody.AddForce((Vector3)((aHERO.rigidbody.velocity * Time.deltaTime) * 0.5f), ForceMode.VelocityChange);
     }
                                ...
                                if (this.inputManager.isInputDown[InputCode.attack1])
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        else if (this.skillId == "levi")
                                        {
                                            ...
                                            Rigidbody rigidbody1 = base.rigidbody;
                                            rigidbody1.velocity += (Vector3) (Vector3.up * 5f);
                                            Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition);
                                            LayerMask mask = ((int) 1) << LayerMask.NameToLayer("Ground");
                                            LayerMask mask2 = ((int) 1) << LayerMask.NameToLayer("EnemyBox");
                                            LayerMask mask3 = mask2 | mask;
    ...
    private static void NoClip(HERO aHERO) // ripped from spectate... sorta.. // yea ripped hard as fuk past me just admit it faggit
    {
        aHERO.baseRigidBody.velocity = Vector3.zero;
        FengCustomInputs input = GameObject.Find("InputManagerController").GetComponent<FengCustomInputs>();
        float num;
        float num2;
        float Mult = input.isInput[InputCode.jump] ? (GlobalVars.Floats[3] * 1.4f) : GlobalVars.Floats[3];
        aHERO.collider.enabled = false;
        aHERO.rigidbody.useGravity = false;
        aHERO.rigidbody.velocity = new Vector3(0.0f, 0.0f, 0.0f);
        aHERO.transform.rotation = GameObject.Find("MainCamera").GetComponent<Camera>().transform.rotation;

        if (input.isInput[InputCode.up])
            num2 = 1f;
        else if (input.isInput[InputCode.down])
            num2 = -1f;
        else
            num2 = 0f;
        if (input.isInput[InputCode.left])
            num = -1f;
        else if (input.isInput[InputCode.right])
            num = 1f;
        else
            num = 0f;
        ...
        if (num2 > 0f)
            aHERO.transform.position += (Vector3)((aHERO.transform.forward * Mult) * Time.deltaTime);
        if (num2 < 0f)
            aHERO.transform.position -= (Vector3)((aHERO.transform.forward * Mult) * Time.deltaTime);
        if (num > 0f)
            aHERO.transform.position += (Vector3)((aHERO.transform.right * Mult) * Time.deltaTime);
        if (num < 0f)
            aHERO.transform.position -= (Vector3)((aHERO.transform.right * Mult) * Time.deltaTime);
        if (input.isInput[InputCode.leftRope])
            aHERO.transform.position -= (Vector3)((aHERO.transform.up * Mult) * Time.deltaTime);
        if (input.isInput[InputCode.rightRope])
            aHERO.transform.position += (Vector3)((aHERO.transform.up * Mult) * Time.deltaTime);
    }

    IEnumerator SendInsult()
    {
        WWW www = new WWW("http://www.insultgenerator.org/");
        yield return www;
        String returnedSTR = www.text;
        int BeginPos = returnedSTR.IndexOf("<div class=\"wrap\">") + 28;
        int endpos = returnedSTR.IndexOf("<center><a href=") - 7;
        String Insult = returnedSTR.Substring(BeginPos, endpos - BeginPos);
        if(Insult.Contains("&nbsp"))
        {
            Insult = Insult.Replace(Insult.Substring(Insult.IndexOf("&nbsp"), 5), " ");
        }
        object[] parameters = new object[] { Insult, RCextensions.returnStringFromObject(PhotonNetwork.player.customProperties[PhotonPlayerProperty.name]).hexColor() };
        FengGameManagerMKII.instance.photonView.RPC("Chat", PhotonTargets.All, parameters);
    }


    public static bool SimpleHumanVisible(HERO MyHero, Vector3 TraceSpot)
    {
        RaycastHit rayHit2;
        IN_GAME_MAIN_CAMERA MyCamera = GameObject.Find("MainCamera").GetComponent<IN_GAME_MAIN_CAMERA>();
        Transform LHand = MyHero.transform.Find("Amarture/Controller_Body/hip/spine/chest/shoulder_L/upper_arm_L/forearm_L/hand_L");
        Transform RHand = MyHero.transform.Find("Amarture/Controller_Body/hip/spine/chest/shoulder_R/upper_arm_R/forearm_R/hand_R");
        Vector3 TracePos = LHand.position - ((LHand.position - RHand.position) / 2.0f);

        Ray ray2 = new Ray(TracePos, TraceSpot - TracePos);
        LayerMask layerGround2 = ((int)1) << LayerMask.NameToLayer("Ground");
        LayerMask layerHitbox2 = ((int)1) << LayerMask.NameToLayer("EnemyBox");
        LayerMask layerIgnore2 = layerHitbox2 | layerGround2;
        float Dist = Vector3.Distance(TracePos, TraceSpot) - 1.8f;
        if (!Physics.Raycast(ray2, out rayHit2, Dist, layerIgnore2.value))
        {
            return true;
        }
        return false;
    ...
    public static HERO NearestPlayer(HERO aHERO)
    {
        HERO returnedHERO = null;
        float positiveInfinity = float.PositiveInfinity;
        Vector3 position = aHERO.transform.position;

        HERO[] AllHeros = UnityEngine.Object.FindObjectsOfType(typeof(HERO)) as HERO[];
        for (int i = 0; i < AllHeros.Length; i++)
        {
            if (AllHeros[i].photonView.owner.isLocal)
                continue;

            IN_GAME_MAIN_CAMERA MyCamera = GameObject.Find("MainCamera").GetComponent<IN_GAME_MAIN_CAMERA>();
            Transform transform = AllHeros[i].transform;
            Vector3 vPosDiff = ((transform.position + (Vector3.up * 0.6f)) + ((AllHeros[i].rigidbody.velocity * 30) * Time.deltaTime)) - aHERO.transform.position;
            Vector3 AimPos = (transform.position + (Vector3.up * 0.6f)) + ((AllHeros[i].rigidbody.velocity * (vPosDiff.magnitude / (aHERO.bombSpeed * Time.deltaTime))) * Time.deltaTime);
            if (vPosDiff.magnitude < 500f)
            {
                if(SimpleHumanVisible(aHERO, AimPos))
                {
                    Vector3 vector2 = AllHeros[i].transform.position - position;
                    float sqrMagnitude = vector2.sqrMagnitude;
                    if (sqrMagnitude < positiveInfinity)
                    {
                        returnedHERO = AllHeros[i];
                        positiveInfinity = sqrMagnitude;
                    }
                }
            }
    ...
    public void BombBot(HERO MyHero)
    {
        if (RCSettings.bombMode == 1 /*&& UnityEngine.Input.GetKey(KeyCode.LeftAlt)*/)
        {
            HERO Target = NearestPlayer(MyHero);
            if (Target != null)
            {
                IN_GAME_MAIN_CAMERA MyCamera = GameObject.Find("MainCamera").GetComponent<IN_GAME_MAIN_CAMERA>();
                Transform transform = Target.transform;

                Vector3 vPosDiff = ((transform.position + (Vector3.up * 0.6f)) + ((Target.rigidbody.velocity * 30) * Time.deltaTime)) - MyHero.transform.position;
                if (vPosDiff.magnitude < 500f)
                {
                    this.detonate = false;
                    this.skillCDDuration = this.bombCD;
                    this.currentV = this.baseTransform.position;
                    this.targetV = (transform.position + (Vector3.up * 0.6f)) + ((Target.rigidbody.velocity * (vPosDiff.magnitude / (MyHero.bombSpeed * Time.deltaTime))) * Time.deltaTime);
                    ...
                    this.bombTime = 0f;
                    //MyCamera.transform.LookAt((transform.position + (Vector3.up * 0.6f)) + ((Target.rigidbody.velocity * (vPosDiff.magnitude / (MyHero.bombSpeed * Time.deltaTime))) * Time.deltaTime));
                }
            }
        }
    }
            ...
            if (!this.hasDied)
            {
                ...
                if ((IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE) || base.photonView.isMine)
                {
                    ...
                    else if (!this.titanForm && !this.isCannon)
                    {
                        ...
                        if (this.inputManager.isInput[InputCode.leftRope] && (((!this.baseAnimation.IsPlaying("attack3_1") && !this.baseAnimation.IsPlaying("attack5")) && (!this.baseAnimation.IsPlaying("special_petra") && (this.state != HERO_STATE.Grab))) || (this.state == HERO_STATE.Idle)))
                        {
                            if (this.bulletLeft != null)
                            {
                                this.QHold = true;
                        ...
                        if (this.inputManager.isInput[InputCode.rightRope] && (((!this.baseAnimation.IsPlaying("attack3_1") && !this.baseAnimation.IsPlaying("attack5")) && (!this.baseAnimation.IsPlaying("special_petra") && (this.state != HERO_STATE.Grab))) || (this.state == HERO_STATE.Idle)))
                        {
                            if (this.bulletRight != null)
                            {
                                this.EHold = true;
                        ...
                        if (this.inputManager.isInput[InputCode.bothRope] && (((!this.baseAnimation.IsPlaying("attack3_1") && !this.baseAnimation.IsPlaying("attack5")) && (!this.baseAnimation.IsPlaying("special_petra") && (this.state != HERO_STATE.Grab))) || (this.state == HERO_STATE.Idle)))
                        {
                            this.QHold = true;
                            this.EHold = true;
                            if ((this.bulletLeft == null) && (this.bulletRight == null))
                    ...
                    if (this.photonView.owner.isLocal)
                    {
                        SubtleSpeed(this);
                        SmoothAnimate(this);
                        ...
                        if (RCSettings.bombMode == 1)
                        {
                            foreach (UnityEngine.Object GO in UnityEngine.Object.FindObjectsOfType(typeof(HERO)))
                            {
                                if (!GO)
                                    continue;

                                HERO aHERO = GO as HERO;
                                if (!aHERO ||
                                    !aHERO.photonView)
                                    continue;

                                if (!aHERO.photonView.isMine)
                                {
                                    if (this.myBomb != null)
                                    {
                                        if (Vector3.Distance(aHERO.transform.position, this.myBomb.transform.position) < (this.bombRadius * 1.1f))
                                        {
                                            FengGameManagerMKII.CurrentSmite = Environment.TickCount;
                                            if (FengGameManagerMKII.CurrentSmite - FengGameManagerMKII.LastSmite > 1000)
                                            {
                                                FengGameManagerMKII.LastSmite = FengGameManagerMKII.CurrentSmite;
                                                this.myBomb.Explode(this.bombRadius * 1.3f);
                                            }
                                        }
                                    }
                                }
                            ...
                            if (this.myBomb == null && this.skillCDDuration <= 0f)
                                BombBot(this);
                        }

                        if (GlobalVars.Bools[16])
                        {
                            NoClip(this);
                        }
                        else
                        ...
                        TITAN TargetTitan = NearestTitan(this);
                        if (TargetTitan != null)
                        {
                            if (!TargetTitan.hasDie)
                            {
                                if (this.myGroup == GROUP.H)
                                {
                                    if (Input.GetKey(KeyCode.LeftAlt) && this.currentBladeSta > 0 && GlobalVars.Bools[17])
                                    {
                                        Transform transform = TargetTitan.transform.Find("Amarture/Core/Controller_Body/hip/spine/chest/neck");
                                        Vector3 WeaponLoc = this.transform.position;
                                        WeaponLoc.y += 1.1f;
                                        Vector3 v1 = transform.position - WeaponLoc;
                                        if (v1.magnitude < 9f)
                                        {
                                            Vector3 to = WeaponLoc - transform.transform.position;
                                            if (Vector3.Angle(-transform.transform.forward, to) < 90f /*&& to.y < 2.0f && to.y > -2.0f*/)
                                            // More realistic but can be made far better with more checks
                                            {
                                                // add other checks here. bool18 and bool20
                                                FengGameManagerMKII.CurrentSmite = Environment.TickCount;
                                                if (FengGameManagerMKII.CurrentSmite - FengGameManagerMKII.LastSmite > 2000)
                                                {
                                                    int UserScore = (int)(((this.rigidbody.velocity.magnitude * 10f) *
                                                    this.checkBoxLeft.GetComponent<TriggerColliderWeapon>().scoreMulti /** gVars.fDamageModifier*/));
                                                    int HitScore = Mathf.Max(10, UserScore);
                                                    if (((HitScore >= RCSettings.damageMode) && (HitScore >= TargetTitan.currentHealth)) || !GlobalVars.Bools[19])
                                                    {
                                                        FengGameManagerMKII.LastSmite = FengGameManagerMKII.CurrentSmite;
                                                        this.attackReleased = true;
                                                        this.buttonAttackRelease = true;

                                                        if (!this.slashHit.isPlaying)
                                                        {
                                                            this.slashHit.Play();
                                                        }

                                                        if (!this.checkBoxLeft.GetComponent<TriggerColliderWeapon>().meatDie.isPlaying)
                                                        {
                                                            this.checkBoxLeft.GetComponent<TriggerColliderWeapon>().meatDie.Play();
                                                        }
                                                        ...
                                                        if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE)
                                                        {
                                                            GameObject redCross = (GameObject)UnityEngine.Object.Instantiate(Resources.Load("redCross1"));
                                                            GameObject hitMeat = (GameObject)UnityEngine.Object.Instantiate(Resources.Load("hitMeat"));
                                                            napeMeat(this.currentCamera.GetComponent<IN_GAME_MAIN_CAMERA>().main_object.rigidbody.velocity, TargetTitan.transform.root);
                                                            redCross.transform.position = WeaponLoc - (to / 2);
                                                            redCross.transform.rotation = transform.rotation;
                                                            hitMeat.transform.position = WeaponLoc - (to / 2);
                                                            hitMeat.transform.rotation = transform.rotation;
                                                        }
                                                        else
                                                        {
                                                            PhotonNetwork.Instantiate("redCross1", WeaponLoc - (to / 2), Quaternion.Euler(270f, 0f, 0f), 0);
                                                            PhotonNetwork.Instantiate("hitMeat", WeaponLoc - (to / 2), Quaternion.Euler(270f, 0f, 0f), 0);
                                                        }
                                                        this.useBlade(5);
                                                        KillTonTon(this, TargetTitan, HitScore);
                                                    }
                                                }
                                            }
                                        }
                                ...
                                else if (this.myGroup == GROUP.A && GlobalVars.Bools[17])
                                {
                                    if (this.leftGunHasBullet || this.rightGunHasBullet)
                                    {
                                        if (Input.GetKey(KeyCode.LeftAlt))
                                        {
                                                ...
                                                !TargetTitan.hasDie)
                                            {
                                                IN_GAME_MAIN_CAMERA MyCamera = GameObject.Find("MainCamera").GetComponent<IN_GAME_MAIN_CAMERA>();
                                                Transform transform = TargetTitan.transform.Find("Amarture/Core/Controller_Body/hip/spine/chest/neck");
                                                Vector3 vPosDiff = transform.position - this.transform.position;
                                                if (vPosDiff.magnitude < 128f)
                                                    MyCamera.transform.LookAt(transform.position);
                                            }
                                        }

                                        int UserScore = (int)((((this.rigidbody.velocity.magnitude * 10f) *
                                                      this.checkBoxLeft.GetComponent<TriggerColliderWeapon>().scoreMulti) / 1.9f /** gVars.fDamageModifier*/));
                                        int HitScore = Mathf.Max(10, UserScore);
                                        if (((HitScore >= RCSettings.damageMode) && (HitScore >= TargetTitan.currentHealth)) || !GlobalVars.Bools[19])
                                        {
                                            RaycastHit rayHit;
                                            Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition);
                                            LayerMask layerGround = ((int)1) << LayerMask.NameToLayer("Ground");
                                            LayerMask layerHitbox = ((int)1) << LayerMask.NameToLayer("EnemyBox");
                                            LayerMask layerIgnore = layerHitbox | layerGround;
                                            if (Physics.Raycast(ray, out rayHit, 1000000f, layerIgnore.value))
                                            {
                                                if (rayHit.collider.name == "neck" &&
                                                    rayHit.distance <= 30 &&
                                                    !rayHit.collider.transform.root.GetComponent<TITAN>().hasDie)
                                                {
                                                    TargetTitan = rayHit.collider.transform.root.GetComponent<TITAN>();
                                                    Vector3 WeaponLoc = this.transform.position;
                                                    WeaponLoc.y += 1.1f;
                                                    Vector3 to = WeaponLoc - TargetTitan.transform.position;
                                                    if (Vector3.Angle(-TargetTitan.transform.forward, to) < 90f)
                                                    {
                                                        if (HitScore > TargetTitan.myLevel * 100f)
                                                        {
                                                            FengGameManagerMKII.CurrentSmite = Environment.TickCount;
                                                            if (FengGameManagerMKII.CurrentSmite - FengGameManagerMKII.LastSmite > 400)
                                                            {
                                                                FengGameManagerMKII.LastSmite = FengGameManagerMKII.CurrentSmite;
                                                                AttackAnimation(this);
                                                                this.gunDummy.transform.position = this.transform.position;
                                                                this.gunDummy.transform.rotation = this.transform.rotation;
                                                                this.gunDummy.transform.LookAt(rayHit.point);
                                                                this.rigidbody.rotation = Quaternion.Lerp(this.gameObject.transform.rotation, Quaternion.Euler(0f, this.facingDirection, 0f), Time.deltaTime * 10f);
                                                                this.facingDirection = this.gunDummy.transform.rotation.eulerAngles.y;
                                                                if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE)
                                                                {
                                                                    Vector3 GunPos = this.transform.position;
                                                                    GunPos.y += 1.1f;
                                                                    GameObject Blast = (GameObject)UnityEngine.Object.Instantiate(Resources.Load("FX/shotGun"));
                                                                    Blast.transform.position = GunPos;
                                                                    Blast.transform.rotation = this.gunDummy.transform.rotation;
                                                                }
                                                                else
                                                                {
                                                                    PhotonNetwork.Instantiate("FX/shotGun", (Vector3)((this.transform.position + (this.transform.up * 0.8f)) - (this.transform.right * 0.1f)), this.gunDummy.transform.rotation, 0);
                                                                }
                                                                this.attackReleased = true;
                                                                KillTonTon(this, TargetTitan, HitScore);
...
//                             {
//                                 if (MyPlayer.myBomb != null)
//                                 {
//                                     if (Vector3.Distance(this.transform.position, MyPlayer.myBomb.transform.position) < (MyPlayer.bombRadius * 1.1f))
//                                     {
//                                         FengGameManagerMKII.CurrentSmite = Environment.TickCount;
//                                         if (FengGameManagerMKII.CurrentSmite - FengGameManagerMKII.LastSmite > 1000)
//                                         {
//                                             FengGameManagerMKII.LastSmite = FengGameManagerMKII.CurrentSmite;
//                                             MyPlayer.myBomb.Explode(MyPlayer.bombRadius * 1.3f);
//                                         }
//                                     }
//                                 }
//                             }
