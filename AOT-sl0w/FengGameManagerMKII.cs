...
public class FengGameManagerMKII : Photon.MonoBehaviour
{
    ...
    public static string UpdateStatus = "Welcome to sl0wm0d.";
    public static int CurrentSmite;
    public static int LastSmite;
//     public static int CurrentKick = Environment.TickCount;
//     public static int LastKick = 0;
    public static bool bIgnoreLeaves;
    public static bool DownloadDLL = false;
    public static bool DownloadAsset = false;
    public static bool HasToDownload = false;
    public static bool DLLReady = false;
    public static bool AssetReady = false;
    public static string ModVersion = "1.1";
    public static bool StillCrashingMe = false;
    ...
    public bool bannoy = false;
    public int idtoannoy = 0;
    ...
    public int wave = 1;
    public List<PhotonPlayer> InternalIgnore;
    public static bool HasJustJoined = false;


    public void InternalIgnorePlayer(PhotonPlayer pPlayer, string Reason)
    {
        if (!GlobalVars.Bools[30])
            return;

        if (!InternalIgnore.Contains(pPlayer))
        {
             object[] parameters = new object[] { String.Format("<color=#FFFFFF>[</color><b><color=#A5D2FF>sl0wm0d</color></b><color=#FFFFFF>]</color>{0}-ID:{1} has been internally ignored\n<color=#FFFFFF>[</color><b><color=#FFD2A5>REASON</color></b><color=#FFFFFF>]</color>: {2}.", RCextensions.returnStringFromObject(pPlayer.customProperties[PhotonPlayerProperty.name]).hexColor(), pPlayer.ID, Reason), string.Empty };
             FengGameManagerMKII.instance.photonView.RPC("Chat", PhotonTargets.All, parameters);

            if (!pPlayer.isLocal)
            {
                InternalIgnore.Add(pPlayer);
            }
        }
    }


    public IEnumerator EnforceViolations()
    {
        while (true)// I hate the idea of using invokerepeating. Fight me.
        {
            yield return new WaitForSeconds(1f);
            for (int player = 0; player < PhotonNetwork.playerList.Length; player++)
            {
                PhotonPlayer pPlayer = PhotonNetwork.playerList[player];

                if (pPlayer.isMasterClient)
                    pPlayer.ViolationCount /= 4;

                if (!pPlayer.isLocal && pPlayer.ViolationCount > PhotonNetwork.GetPing() / 2f )//25
                    InternalIgnorePlayer(pPlayer, "Exceeded Violations allowed");
                else
                    pPlayer.ViolationCount = 0;
            }
        }
    ...
    private void clearlevel(string[] link, int gametype, PhotonMessageInfo info)
    {
        ...
        {
            InternalIgnorePlayer(info.sender, "(RPC)clearlevel");
        }
    }
            ...
            if (this.timeElapse > 1f)
            {
                ...
                else if (IN_GAME_MAIN_CAMERA.gamemode != GAMEMODE.CAGE_FIGHT)
                {
                    ...
                    else if (IN_GAME_MAIN_CAMERA.gamemode == GAMEMODE.PVP_AHSS)
                    {
                        ...
                        {
                            str2 = str3;
                            object[] objArray12 = new object[] { str2, (j == 0) ? string.Empty : " : ", "Team", j + 1, " ", this.teamScores[j], string.Empty };
        ...
        if (!flag && !flag2)
        {
            for (num = 0; num < content.Length; num++)
            {
                ...
                if (strArray[0].StartsWith("custom"))
                {
                    ...
                    if (strArray[6] != "0")
                    {
                        color = new UnityEngine.Color(Convert.ToSingle(strArray[7]), Convert.ToSingle(strArray[8]), Convert.ToSingle(strArray[9]), num2);
                        foreach (MeshFilter filter in obj2.GetComponentsInChildren<MeshFilter>())
                        {
                            mesh = filter.mesh;
                            colorArray = new UnityEngine.Color[mesh.vertexCount];
                            num8 = 0;
    ...
    private void customlevelRPC(string[] content, PhotonMessageInfo info)
    {
        ...
        {
            InternalIgnorePlayer(info.sender, "(RPC)customlevelRPC");
        }
    }
...
//         this.refreshRacingResult2();
        InternalIgnorePlayer(info.sender, "(RPC)getRacingResult");
    }
    ...
    IEnumerator AntiIgnore(PhotonPlayer player)
    {
        FengGameManagerMKII FengMGR = FengGameManagerMKII.instance;
        PhotonNetwork.bWasJustKicked = false;
        FengGameManagerMKII.bIgnoreLeaves = true;
...
//         PhotonNetwork.networkingPeer.OpRaiseEvent(0xfe, null, true, null);
        ...
        PhotonNetwork.player.SetCustomProperties(PhotonNetwork.player.customProperties);
/*        yield return new WaitForSeconds(0.8f);*/
        FengGameManagerMKII.bIgnoreLeaves = false;
        if(FengMGR.myLastHero != null && HERO.MyPlayer != null)
            FengMGR.SpawnPlayerAt3(FengMGR.myLastHero);
        string tagstr = string.Empty;
        tagstr = tagstr + "[" + Convert.ToString(player.ID) + "] ";
        if (player.customProperties.ContainsKey("RCteam"))
        {
            tagstr = tagstr + "[RC] ";
        }
        if (player.customProperties.ContainsKey("NRC"))
        {
            tagstr = tagstr + "[NRC] ";
        }
        if (player.customProperties.ContainsKey("Nathan"))
        {
            tagstr = tagstr + "[Nathan] ";
        }
        if (player.customProperties.ContainsKey("KageNoKishi"))
        {
            tagstr = tagstr + "[KnK] ";
        }
        if (player.customProperties.ContainsKey("not null"))
        {
            tagstr = tagstr + "[ECMod] ";
        }
        if (player.customProperties.ContainsKey("raohsopmod"))
        {
            tagstr = tagstr + "[RAOHMod] ";
        }
        object[] parameters = new object[] { String.Format("<color=#FFFFFF>[</color><color=#FF6464><b>BAN</b></color><color=#FFFFFF>]</color> <color=#FFFFFF>Hey,(</color><color=#6EB1FF>{0}</color> <color=#FFFFFF> USER)</color> <color=#FF6464><b>{1}</b></color><color=#FFFFFF> your game crasher isn't working on me</color> <color=#FFFF00>(づಠᴥಠ)づ</color><color=#FFFFFF>.</color>", tagstr, ModDrawOverlay.RemoveTags((string)player.customProperties[PhotonPlayerProperty.name])), string.Empty };
...
//         PhotonNetwork.player.SetCustomProperties(PhotonNetwork.player.customProperties);
    ...
    private void ignorePlayer(int ID, PhotonMessageInfo info)
    {
        if (info.sender.isMasterClient)
        {
            ...
            {
                base.StartCoroutine(AntiIgnore(info.sender));
        ...
        {
            InternalIgnorePlayer(info.sender, "(RPC)ignorePlayer");
        }
        this.RecompilePlayerList(0.1f);
    }
    ...
    private void ignorePlayerArray(int[] IDS, PhotonMessageInfo info)
    {
        if (info.sender.isMasterClient)
        {
            ...
            {
//                 if (IDS[i] == PhotonNetwork.player.ID)
//                 {
//                     base.StartCoroutine(AntiIgnore(info.sender));
//                     this.RecompilePlayerList(0.1f);
//                     return;
//                 }
        ...
        {
            InternalIgnorePlayer(info.sender, "(RPC)ignorePlayerArray");
        }
        this.RecompilePlayerList(0.1f);
    }
    ...
    private void labelRPC(int setting, PhotonMessageInfo info)
    {
        ...
        if (PhotonView.Find(setting) != null)
        {
            ...
            {
                InternalIgnorePlayer(info.sender, "(RPC)labelRPC");
            }
        }
    }
...
//         ModDrawGUI.cameraObj.GetComponent<Camera>().enabled = true;
        //ModDrawGUI.renderTexture = ModDrawGUI.RTImage(GameObject.Find("MainCamera").GetComponent<Camera>());//ModDrawGUI.cameraObj.GetComponent<Camera>());
//         ModDrawGUI.cameraObj.GetComponent<Camera>().enabled = false;
    }
    ...
    public IEnumerator FloodAll()
    {
        string Aids = "JewsDidThis";
        for (int z = 0; z < 4; z++)
        {
            for (int i = 0; i < 5000; i++) // 800
            {
                object[] parameters = new object[] { Aids, float.MaxValue };
                base.photonView.RPC("getRacingResult", PhotonTargets.OthersBuffered, parameters);
            }
            yield return new WaitForSeconds(0.8f);
        }
    }
    public IEnumerator FloodAllLeave()
    {
        string Aids = "JewsDidThis";
        object[] parameters = new object[] { Aids, float.MaxValue };
        List<PhotonPlayer> ayylmao = new List<PhotonPlayer>();
        for (int i = 0; i < PhotonNetwork.playerList.Length; i++)
        {
            if(PhotonNetwork.playerList[i].customProperties.ContainsKey("not null"))
            {
                ayylmao.Add(PhotonNetwork.playerList[i]);
            }
        }

        for (int i = 0; i < ayylmao.Count; i++)
        {
            object[] parameters2 = new object[] { ayylmao[i], "d340" };
            base.photonView.RPC("SkinCheck", ayylmao[i], parameters2);
        }
    ...
    public IEnumerator FloodID()
    {
        for(int i = 1; i < 10000; i++)
        {
            if (connectcount < 800)
            {
                object[] objArray3 = new object[] { i.ToString(), "" };
...
//                 PhotonNetwork.player.SetCustomProperties(hashtable);
            ...
            else
            {
                ...
                PhotonNetwork.RejoinRoom(CurRoom.name, null, null, false, PhotonNetwork.player.customProperties);
...
//         object[] parameters = new object[] { Aids, float.MaxValue };
//         PhotonPlayer player = PhotonPlayer.Find(ID);
//         UnityEngine.Debug.Log("Flooding user: " + player);
//         for (int z = 0; z < 4; z++)
//         {
//             for (int i = 0; i < 5000; i++) // 800
//             {
//                 base.photonView.RPC("getRacingResult", player, parameters);
//             }
//             yield return new WaitForSeconds(0.8f);
//         }
    }

    public void ripuser( int ID )
    {
        RaiseEventOptions opt = new RaiseEventOptions();
        opt.TargetActors = new int[] { ID };
        PhotonNetwork.networkingPeer.OpRaiseEvent(0xe4, null, true, opt);
    }

    public void FloodAllUsers()
    {
        base.StartCoroutine(FloodAll());
    }

    public void FloodAllAndLeave()
    {
        base.StartCoroutine(FloodAllLeave());
    }

    public void FloodUser()
    {
        base.StartCoroutine(FloodID());
    }

//     public void postimg(string img)
//     {
//         base.StartCoroutine(ChatImage(img));
//     }
            ...
            else if (IN_GAME_MAIN_CAMERA.gamemode != GAMEMODE.CAGE_FIGHT)
            {
                ...
                else if (IN_GAME_MAIN_CAMERA.gamemode == GAMEMODE.SURVIVE_MODE)
                {
                    if (this.checkIsTitanAllDie())
                    {
                        ...
                        if (((RCSettings.maxWave == 0) && (this.wave > 20)) || ((RCSettings.maxWave > 0) && (this.wave > RCSettings.maxWave)))
                        {
                            this.gameWin2();
                        }
                        else
    ...
    public void handle_quit_loading()
    {
    ...
    public void OnGUI()
    {
            ...
            if (isAssetLoaded)
            {
                ...
                if (text != null)
                {
                    if (text.StartsWith("Verifying"))
                    {
                        ...
                        float x = (Screen.width / 2) - 115f;
                        float y = (Screen.height / 2) - 45f;
                        GUI.Box(new Rect(x, y, 230f, 90f), string.Empty);
                        GUI.DrawTexture(new Rect(x + 2f, y + 2f, 226f, 86f), this.textureBackgroundBlack);
                    ...
                    else if (((GameObject.Find("ButtonCREDITS") != null) && (GameObject.Find("ButtonCREDITS").transform.parent.gameObject != null)) && NGUITools.GetActive(GameObject.Find("ButtonCREDITS").transform.parent.gameObject))
                    {
                        ...
                        if (((int) settings[0xbb]) == 1)
                        {
                            ...
                            else
                            {
                                ...
                                nameField = GUI.TextField(new Rect(65f, 80f, 145f, 20f), nameField, 400);
                                GUI.Label(new Rect(20f, 105f, 45f, 20f), "Guild:", "Label");
                                LoginFengKAI.player.guildname = GUI.TextField(new Rect(65f, 105f, 145f, 20f), LoginFengKAI.player.guildname, 400);
                        ...
                        else if (((int) settings[0xbb]) == 0)
                        {
                            ...
                            {
                                GUI.Label(new Rect(20f, 80f, 70f, 20f), "Username:", "Label");
                                usernameField = GUI.TextField(new Rect(90f, 80f, 130f, 20f), usernameField, 400);
                                GUI.Label(new Rect(20f, 105f, 70f, 20f), "Password:", "Label");
                                passwordField = GUI.PasswordField(new Rect(90f, 105f, 130f, 20f), passwordField, '*', 400);
        ...
        else if (IN_GAME_MAIN_CAMERA.gametype != GAMETYPE.STOP)
        {
            ...
            else if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.MULTIPLAYER)
            {
                ...
                {
                    int loaded_object_num = RCextensions.returnStringFromObject(PhotonNetwork.player.customProperties[PhotonPlayerProperty.currentLevel]).Length;
                    int max_object_num = RCextensions.returnStringFromObject(PhotonNetwork.masterClient.customProperties[PhotonPlayerProperty.currentLevel]).Length;
                    string loading_string = "Loading... [" + loaded_object_num.ToString() + "/" + max_object_num.ToString() + "]";
                    Vector2 loading_string_size = PanelMain.TextBounds(loading_string);
                    int screen_width_center = Screen.width / 2;
                    float loadbar = (float)loaded_object_num / (float)max_object_num;
                    int modifier = Screen.width / 3;
                    GUI.DrawTexture(new Rect(screen_width_center - modifier, Screen.height - 27, modifier * 2, 22), this.textureBackgroundBlack);
                    GUI.DrawTexture(new Rect(screen_width_center - modifier + 1, Screen.height - 26, (int)(loadbar * modifier - 2), 20), this.textureBackgroundGrey);
                    GUI.Label(new Rect(screen_width_center - loading_string_size[0] / 2, Screen.height - 24, loading_string_size[0], loading_string_size[1]), loading_string, PanelMain.changelogtextstyle);
                    this.retryTime += Time.deltaTime;
                    Screen.lockCursor = false;
                    Screen.showCursor = true;
                    if (GUI.Button(new Rect(Screen.width - 70f, Screen.height - 27, 65f, 22f), "Quit", PanelMain.changelogstyle))
                    {
                        handle_quit_loading();
                    }
                }
    ...
    {
        //The objects aren't directly destroyed but put in a list because direct destroy can make Unity miss gameobjects
        List<GameObject> goToDestroy = new List<GameObject>();

        GameObject[] gos = GameObject.FindObjectsOfType<GameObject>();
        foreach (GameObject go in gos)
        {
            //Destroy default map's objects
            if ((go.name == "gameobjectOutSide" || go.name.Contains("Cube") || go.name.Contains("HOUSE") || go.name.Contains("TREE") || go.tag == "playerRespawn" || go.tag == "titanRespawn") && (go.tag != "Player" && go.transform.root.tag != "Player" && go.tag != "titan" && go.transform.root.tag != "titan"))
                goToDestroy.Add(go);
            //Just disable the light (apparently it can make the game glitchy :| )
    ...
    public IEnumerator joinedtimer()
    {
    ...
    {
        this.StartCoroutine(joinedtimer());
        ...
        hashtable.Add(PhotonPlayerProperty.RCteam, 0);
        hashtable.Add(PhotonPlayerProperty.currentLevel, string.Empty);
        //hashtable.Add(PhotonPlayerProperty.sl0wmod, string.Empty);
         //hashtable.Add(PhotonPlayerProperty.Nathan, string.Empty);
        //hashtable.Add(PhotonPlayerProperty.Angry_Guest, string.Empty);
        ExitGames.Client.Photon.Hashtable propertiesToSet = hashtable;
        PhotonNetwork.player.SetCustomProperties(propertiesToSet);
        this.humanScore = 0;
        this.titanScore = 0;
...
//             this.name = this.name.Replace("[-]", "");
    ...
    private void OnLevelWasLoaded(int level)
    {
        ...
        {
//             DestroyMap();
//             GameObject mapGo = Instantiate(GlobalVars.mapBundle.Load("GameObject")) as GameObject;
//             mapGo.transform.position = new Vector3(0, 0, 0);
// 
//             GameObject[] everyGameObjects = FindObjectsOfType<GameObject>();
//             foreach (GameObject go in everyGameObjects)
//             {
//                 if (go.name == "supply")
//                 {
//                     GameObject supply = PhotonNetwork.Instantiate("aot_supply", go.transform.position, go.transform.rotation, 0);
//                     supply.transform.localScale = go.transform.localScale;
...
//             }
            ...
            bloom.screenBlendShader = GlobalVars.BloomShader;
            bloom.blurAndFlaresShader = GlobalVars.BlurFlaresShader;
            bloom.brightPassFilterShader = GlobalVars.BrightPassFilter;
            bloom.lensFlareShader = GlobalVars.LensFlareCreate;
            bloom.tweakMode = Bloom.TweakMode.Complex;
            bloom.screenBlendMode = Bloom.BloomScreenBlendMode.Add;
            bloom.bloomThreshholdColor = Color.white;
            bloom.bloomIntensity = 0.630f;
            bloom.bloomThreshhold = 0.35f;
...
//         }
    ...
    private byte clamp_float_byte(float _in)
    {
        _in = Mathf.Clamp01(_in);
        return (byte)(_in * 255);
    }
    // fucking cant use params as input and output like C. C# not even once.
    private void format_rgb_hex(Color _color, out string _str)
    {
        _str = string.Format("#{0:X2}{1:X2}{2:X2}", clamp_float_byte(_color.r), clamp_float_byte(_color.g), clamp_float_byte(_color.b));
    }

    public IEnumerator chat_image_generator( string _path )
    {
        // i recommend adding checks for png and jpg and fallbacking to DXT1 if not found. https://docs.unity3d.com/ScriptReference/TextureFormat.html
        WWW link = new WWW(_path);                                                          // create a new WWW connection
        yield return link;                                                                  // return if the link isnt loaded
        Texture2D image = new Texture2D(link.texture.width,                                 // create a texture2d
            link.texture.height, TextureFormat.ARGB32, false);                                // see link above for better color configuration and filtering ^ (do yourself)
        link.LoadImageIntoTexture(image);                                                   // load the www image into our texture above
        image.Apply();                                                                      // call apply to lock in the changes
        link.Dispose();                                                                     // dispose of the WWW as we dont need it anymore

        string chatSTR =                                                                    // rip. "@" is only for string literals.
        "<color=#505050>[</color>"
        +"<color=#FFFFFF>sl0img</color>"
        +"<color=#505050>]</color>"
        +"\n"
        +"<size=2>";// play with size you want to use.

        Color LastColor = new Color(300, 300, 300);
        bool bHasTag = false;
        bool bfirst = false;
        for (int y = 1; y < image.height; y++)                                              // iterate height
            {
                for (int x = 0; x < image.width; x++)                                       // iterate width
                {
                    Color curPixel = image.GetPixel(x, image.height - y);
                    string PixelColor;
                    format_rgb_hex(curPixel, out PixelColor);

                    chatSTR = string.Concat(chatSTR + "<color=" + PixelColor + ">██</color>"); // fix when brain works again.'
//                     if (bHasTag)
//                     {
//                         chatSTR = string.Concat(chatSTR + "</color>");
//                         chatSTR = string.Concat(chatSTR + "<color=" + PixelColor + ">██"); // fix when brain works again.'
//                         bHasTag = true;
//                     }
//                     else
//                     {
//                         if(!bfirst)
//                         {
//                             chatSTR = string.Concat(chatSTR + "<color=" + PixelColor + ">██");
//                             bfirst = true;
//                             bHasTag = true;
//                         }
//                         chatSTR = string.Concat(chatSTR + "██");
//                     }
// 
//                     if(x + 1 < image.width)
//                     {
//                         Color nextPixel = image.GetPixel(x + 1, image.height - y);
//                         if (Mathf.Abs(curPixel.r - nextPixel.r) < 40 && Mathf.Abs(curPixel.g - nextPixel.g) < 40 && Mathf.Abs(curPixel.b - nextPixel.b) < 40 && bHasTag)
//                         {
//                             bHasTag = false;
//                         }
//                         else
...
//                     }
                ...
                chatSTR = string.Concat(chatSTR + "\n");                                    // we're done iterating the current line lets go to the next
            }
        //chatSTR = string.Concat(chatSTR + "</color>");   
        chatSTR = string.Concat(chatSTR + "</size>");                                       // close out our size
        object[] param = new object[] { chatSTR, string.Empty };                            // setup params
        FengGameManagerMKII.instance.photonView.RPC("Chat", PhotonTargets.All, param);      // send it in chat

//         object[] param2 = new object[] { chatSTR.Length.ToString(), string.Empty };           // setup params
//         FengGameManagerMKII.instance.photonView.RPC("Chat", PhotonTargets.All, param2);      // send it in chat
    }
    ...
    IEnumerator SpamTitanKills(int ID)
    {
        for(int i = 0; i < 10; i++)
        {
            ...
            if (TargetPlayer != null)
            {
                for (int z = 0; z < 300; z++)
                {
                    object[] parameters = new object[] { TargetPlayer, UnityEngine.Random.Range(10, 2000), (string)PhotonNetwork.player.customProperties[PhotonPlayerProperty.name]};
                    FengGameManagerMKII.instance.photonView.RPC("titanGetKill", TargetPlayer, parameters);
                }
            }
            else 
            {
    ...
    public void spamTitanKill(int ID)
    {
        base.StartCoroutine(SpamTitanKills(ID));
    }

    IEnumerator SpamJihad(int ID)
    {
        for(int i = 0; i < 20; i++)
        {
           FengGameManagerMKII.instance.photonView.RPC("AudioRPC", PhotonPlayer.Find(ID), new object[0]);
            yield return new WaitForSeconds(0.8f);
        }
    }

    public void JihadSLB(int ID)
    {
        base.StartCoroutine(SpamJihad(ID));
    }

    public void scoreboardAll()
    {
        object[] refreshstatusobj = new object[] { this.humanScore, this.titanScore, this.wave, this.highestwave, 0.0f/*this.roundTime*/, /*this.timeTotalServer*/99999.0f, this.startRacing, this.endRacing };
        FengGameManagerMKII.instance.photonView.RPC("refreshStatus", PhotonTargets.MasterClient, refreshstatusobj);// other
        FengGameManagerMKII.instance.photonView.RPC("RequireStatus", PhotonTargets.MasterClient, new object[0]);
    }

    public void scoreboardPlayer(int ID)
    {
        object[] refreshstatusobj = new object[] { this.humanScore, this.titanScore, this.wave, this.highestwave, 0.0f/*this.roundTime*/, /*this.timeTotalServer*/99999.0f, this.startRacing, this.endRacing };
        FengGameManagerMKII.instance.photonView.RPC("refreshStatus", PhotonPlayer.Find(ID), refreshstatusobj);// other
        FengGameManagerMKII.instance.photonView.RPC("RequireStatus", PhotonPlayer.Find(ID), new object[0]);
    }
...
//         }
    ...
    public void SpawnPlayerAt3(string id)
    {
        Vector3 Velocity = GameObject.Find("MainCamera").GetComponent<IN_GAME_MAIN_CAMERA>().main_object.rigidbody.velocity;
        IN_GAME_MAIN_CAMERA component = GameObject.Find("MainCamera").GetComponent<IN_GAME_MAIN_CAMERA>();
        Quaternion AngleReset = component.transform.rotation;
        Vector3 CameraPos = component.transform.position;
        Transform pos = HERO.MyPlayer.gameObject.transform;
        HERO.MyPlayer.netRemoveMe(-1);
        if (!logicLoaded || !customLevelLoaded)
        {
            this.NOTSpawnPlayerRC(id);
        }
        else
        {
            ...
            if (IN_GAME_MAIN_CAMERA.gametype == GAMETYPE.SINGLE)
            {
                if (IN_GAME_MAIN_CAMERA.singleCharacter == "TITAN_EREN")
                {
                    component.setMainObject((GameObject)UnityEngine.Object.Instantiate(Resources.Load("TITAN_EREN"), pos.position, pos.rotation), false, false);
                }
                else
                {
                    component.setMainObject((GameObject)UnityEngine.Object.Instantiate(Resources.Load("AOTTG_HERO 1"), pos.position, pos.rotation), false, false);
                    if (((IN_GAME_MAIN_CAMERA.singleCharacter == "SET 1") || (IN_GAME_MAIN_CAMERA.singleCharacter == "SET 2")) || (IN_GAME_MAIN_CAMERA.singleCharacter == "SET 3"))
                    {
                        HeroCostume costume = CostumeConeveter.LocalDataToHeroCostume(IN_GAME_MAIN_CAMERA.singleCharacter);
                        costume.checkstat();
            ...
            {
                component.setMainObject(PhotonNetwork.Instantiate("AOTTG_HERO 1", position, pos.rotation, 0), true, false);
                id = id.ToUpper();
                if (((id == "SET 1") || (id == "SET 2")) || (id == "SET 3"))
                {
                    HeroCostume costume2 = CostumeConeveter.LocalDataToHeroCostume(id);
        ...
        component.transform.rotation = AngleReset;
        component.transform.position = CameraPos;
        GameObject.Find("MainCamera").GetComponent<IN_GAME_MAIN_CAMERA>().main_object.rigidbody.AddForce(Velocity, ForceMode.VelocityChange);// = Velocity;
    }
    ...
    IEnumerator Annoy()
    {
        while (true)
        {
            ...
            if(bannoy)
            {
                object[] loliobj = new object[] { "snd_eren_roar" };
                FengGameManagerMKII.instance.photonView.RPC("playsoundRPC", PhotonPlayer.Find(idtoannoy), loliobj);
            }
        }
    }
    ...
    private void Start()
    {
        ...
        this.InternalIgnore = new List<PhotonPlayer>();
        ...
        if (this.textureBackgroundGrey == null)
        {
            ...
            this.textureBackgroundGrey.SetPixel(0, 0, new Color(0.294f, 0.294f, 0.294f, 1f));
        ...
        if (!GlobalVars.bOnce)
        {
            ...
            string URL = "File://" + Application.dataPath + "/sl0wAssets.unity3d";
            string URL2 = "File://" + Application.dataPath + "/GameObject.unity3d";
            WWW ayylmao = WWW.LoadFromCacheOrDownload(URL, 72);
            WWW mapload = WWW.LoadFromCacheOrDownload(URL2, 26);
            AssetBundle sl0wBundle = ayylmao.assetBundle;
            GlobalVars.mapBundle = mapload.assetBundle;
            GlobalVars.Segoe = (Font)sl0wBundle.Load("segoe");
            GlobalVars.OutlineShader = (Material)sl0wBundle.Load("outline");
            GlobalVars.BloomShader = (Shader)sl0wBundle.Load("BlendForBloom");
            GlobalVars.BlurFlaresShader = (Shader)sl0wBundle.Load("BlurAndFlares");
            GlobalVars.BrightPassFilter = (Shader)sl0wBundle.Load("BrightPassFilter2");
            GlobalVars.LensFlareCreate = (Shader)sl0wBundle.Load("LensFlareCreate");
            GlobalVars.sl0wTextures = new Texture2D[]
            { 
                (Texture2D)sl0wBundle.Load("sl0wm0dmenu"), 
                (Texture2D)sl0wBundle.Load("sl0wm0dskinmenu"), 
                (Texture2D)sl0wBundle.Load("changelog"),
                (Texture2D)sl0wBundle.Load("changelogbutton")
            };
            GlobalVars.InitVars();
            MyGameObj.AddComponent<ModDrawController>();
            ModDrawController.InitVars();
            MyGameObj.AddComponent<ModDrawOverlay>();
            MyGameObj.AddComponent<ModDrawGUI>();
            MyGameObj.AddComponent<StaticCoroutine>();
            UnityEngine.Object.DontDestroyOnLoad(MyGameObj);
            GlobalVars.bOnce = true;
        }
        LastSmite = Environment.TickCount;
        base.StartCoroutine(Annoy());
        base.StartCoroutine(EnforceViolations());
    }

    IEnumerator DelayRips(string name)
    {
        FengGameManagerMKII FengMGR = FengGameManagerMKII.instance;
        PhotonNetwork.bWasJustKicked = false;
        FengGameManagerMKII.bIgnoreLeaves = true;
        ...
        FengGameManagerMKII.bIgnoreLeaves = false;
        /*FengMGR.SpawnPlayerAt3(FengMGR.myLastHero);*/
        object[] parameters = new object[] { String.Format("<color=#FFFFFF>[</color><color=#FF6464><b>BAN</b></color><color=#FFFFFF>]</color> <color=#FFFFFF>Hey, </color><color=#6EB1FF>[NRCMod User]</color> <color=#FF6464><b>{0}</b></color> <color=#FFFFFF>your game crasher isn't working on me</color> <color=#FFFFFF>(づಠᴥಠ)づ</color><color=#FFFFFF>.</color>", ModDrawOverlay.RemoveTags(name)), string.Empty };
        base.photonView.RPC("Chat", PhotonTargets.All, parameters);
    }
    ...
    public void titanGetKill(PhotonPlayer player, int Damage, string name, PhotonMessageInfo info)
    {
        ...
        object[] parameters2 = new object[] { Damage };
        base.photonView.RPC("netShowDamage", player, parameters2);
    ...
    [RPC]
    public void Ip(PhotonMessageInfo info)
    {
        return;
    }
    ...
    public void titanGetKillbyServer(int Damage, string name)
    {
        ...
        this.netShowDamage(Damage, null);
    ...
    IEnumerator SendInsult()
    {
        ...
        {
            Insult = Insult.Replace(Insult.Substring(Insult.IndexOf("&nbsp"), 6), " ");
        }
        if (Insult.Contains("&#44"))
        {
            Insult = Insult.Replace(Insult.Substring(Insult.IndexOf("&#44"), 5), ",");
        }
        object[] parameters = new object[] { String.Format("<color=#FFC48A>[</color><color=#8AFF8A><b>INSULTGEN</b></color><color=#FFC48A>]</color> <color=#8AC4FF>{0}</color>", Insult), string.Empty };
        FengGameManagerMKII.instance.photonView.RPC("Chat", PhotonTargets.All, parameters);
    }

    private void Update()
    {
//         if (HasToDownload == true && DownloadAsset == false && DownloadDLL == false)
//             Application.Quit();
        ...
        if (this.gameStart)
        {
            ...
            if (GlobalVars.Bools[23])
            {
                GlobalVars.FloatTimers[0] = Environment.TickCount;
                if (GlobalVars.FloatTimers[0] - GlobalVars.FloatTimers[1] > GlobalVars.FloatTimers[2])
                {
                    GlobalVars.FloatTimers[1] = GlobalVars.FloatTimers[0];
                    base.StartCoroutine(SendInsult());
                }
            }
        ...
        if (RCSettings.teamMode == 0)
        {
            foreach (PhotonPlayer player7 in PhotonNetwork.playerList)
            {
                if (player7.customProperties[PhotonPlayerProperty.dead] != null)
                {
                    ...
                    if (!player7.isLocal)
                    {
                        if (player7.customProperties.ContainsKey("RCteam"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[69E6FF][RC] ";
                        }
                        if (player7.customProperties.ContainsKey("NRC"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[FF8269][NRC] ";
                        }
                        if (player7.customProperties.ContainsKey("Nathan"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[CD69FF][Nathan] ";
                        }
                        if (player7.customProperties.ContainsKey("KageNoKishi"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[9BFF69][KnK] ";
                        }
                        if (player7.customProperties.ContainsKey("not null"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[FFFF69][ECMod] ";
                        }
                        if (player7.customProperties.ContainsKey("raohsopmod"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[FF0000][RAOHMod] ";
                        }
                        if (player7.customProperties.ContainsKey("     "))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[FFFFFF][sl0w] ";
                        }
                    }
                    if (player7.isMasterClient)
                    {
        ...
        else
        {
            ...
            foreach (PhotonPlayer player4 in dictionary.Values)
            {
                ...
                if ((player4.customProperties[PhotonPlayerProperty.dead] != null) && (num11 == 1))
                {
                    ...
                    if (!player4.isLocal)
                    {
                        if (player4.customProperties.ContainsKey("RCteam"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[69E6FF][RC] ";
                        }
                        if (player4.customProperties.ContainsKey("NRC"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[FF8269][NRC] ";
                        }
                        if (player4.customProperties.ContainsKey("Nathan"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[CD69FF][Nathan] ";
                        }
                        if (player4.customProperties.ContainsKey("KageNoKishi"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[9BFF69][KnK] ";
                        }
                        if (player4.customProperties.ContainsKey("not null"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[FFFF69][ECMod] ";
                        }
                        if (player4.customProperties.ContainsKey("raohsopmod"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[FF0000][RAOHMod] ";
                        }
                        if (player4.customProperties.ContainsKey("     "))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[FFFFFF][sl0w] ";
                        }
                    }
            ...
            foreach (PhotonPlayer player5 in dictionary2.Values)
            {
                ...
                if ((player5.customProperties[PhotonPlayerProperty.dead] != null) && (num11 == 2))
                {
                    ...
                    if (!player5.isLocal)
                    {
                        if (player5.customProperties.ContainsKey("RCteam"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[69E6FF][RC] ";
                        }
                        if (player5.customProperties.ContainsKey("NRC"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[FF8269][NRC] ";
                        }
                        if (player5.customProperties.ContainsKey("Nathan"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[CD69FF][Nathan] ";
                        }
                        if (player5.customProperties.ContainsKey("KageNoKishi"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[9BFF69][KnK] ";
                        }
                        if (player5.customProperties.ContainsKey("not null"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[FFFF69][ECMod] ";
                        }
                        if (player5.customProperties.ContainsKey("raohsopmod"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[FF0000][RAOHMod] ";
                        }
                        if (player5.customProperties.ContainsKey("     "))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[FFFFFF][sl0w] ";
                        }
                    }
            ...
            foreach (PhotonPlayer player6 in dictionary3.Values)
            {
                ...
                if ((player6.customProperties[PhotonPlayerProperty.dead] != null) && (num11 == 0))
                {
                    ...
                    if (!player6.isLocal)
                    {
                        if (player6.customProperties.ContainsKey("RCteam"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[69E6FF][RC] ";
                        }
                        if (player6.customProperties.ContainsKey("NRC"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[FF8269][NRC] ";
                        }
                        if (player6.customProperties.ContainsKey("Nathan"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[CD69FF][Nathan] ";
                        }
                        if (player6.customProperties.ContainsKey("KageNoKishi"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[9BFF69][KnK] ";
                        }
                        if (player6.customProperties.ContainsKey("not null"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[FFFF69][ECMod] ";
                        }
                        if (player6.customProperties.ContainsKey("raohsopmod"))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[FF0000][RAOHMod] ";
                        }
                        if (player6.customProperties.ContainsKey("     "))
                        {
                            iteratorVariable1 = iteratorVariable1 + "[FFFFFF][sl0w] ";
                        }
                    }
