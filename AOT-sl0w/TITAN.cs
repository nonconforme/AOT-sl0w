...
public class TITAN : Photon.MonoBehaviour
{
    ...
    public void DieByCannon(int viewID)
    {
        ...
        if (view != null)
        {
            ...
            if (this.nonAI)
            {
                FengGameManagerMKII.instance.titanGetKill(view.owner, damage, (string)PhotonNetwork.player.customProperties[PhotonPlayerProperty.name], new PhotonMessageInfo());
            }
            else
            {
                FengGameManagerMKII.instance.titanGetKill(view.owner, damage, base.name, new PhotonMessageInfo());
            }
        }
        else
        {
...
//             return;
        //5-6
/*        level = UnityEngine.Random.Range((float)GlobalVars.Floats[5], (float)GlobalVars.Floats[6]);*/
...
//         object[] parameters = new object[] { level, AI, skinColor };
//         this.photonView.RPC("netSetLevel", PhotonTargets.Others, parameters);
    }
    ...
    private void Start()
    {
        ...
        {
            Renderer CurrentComponent = components[i];

            if (CurrentComponent.name == "hair")
            {
                
//                 Renderer OutlineRender = base.gameObject.AddComponent<MeshRenderer>();
//                 OutlineRender = CurrentComponent;
//                 OutlineRender.material = FengGameManagerMKII.instance.OutlineShader;
                //                 if (gVars.bChams)
                //                 {
                //                     //Chams should still work in RCMod
                //                     Material TestMat = new Material("Shader \"Chams\" { SubShader { Fog { Mode Off } Pass { Color(" + String.Format("{0},{1},{2},{3}", gVars.g_ColArray[1].r, gVars.g_ColArray[1].g, gVars.g_ColArray[1].b, gVars.g_ColArray[1].a) + ") Blend SrcAlpha OneMinusSrcAlpha ZTest Always ZWrite Off } Pass { Color(" + String.Format("{0},{1},{2},{3}", gVars.g_ColArray[0].r, gVars.g_ColArray[0].g, gVars.g_ColArray[0].b, gVars.g_ColArray[0].a) + ") Blend SrcAlpha OneMinusSrcAlpha } } }");
// 
                CurrentComponent.material = GlobalVars.OutlineShader;
            }
        }
    }
        ...
        if (view != null)
        {
            ...
            if (((vector.magnitude < this.lagMax) && !this.hasDie) && ((Time.time - this.healthTime) > 0.2f))
            {
                ...
                if (this.currentHealth < 0f)
                {
                    ...
                    if (this.nonAI)
                    {
                        FengGameManagerMKII.instance.titanGetKill(view.owner, speed, (string)PhotonNetwork.player.customProperties[PhotonPlayerProperty.name], new PhotonMessageInfo());
                    }
                    else
                    {
                        FengGameManagerMKII.instance.titanGetKill(view.owner, speed, base.name, new PhotonMessageInfo());
                    }
                }
                else
                {
