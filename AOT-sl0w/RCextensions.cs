...
public static class RCextensions
{
    ...
    public static Texture2D loadimage2(WWW link, bool mipmap, int size)
    {
        Texture2D tex = new Texture2D(4, 4, TextureFormat.ARGB32, mipmap);
        if (link.size >= size)
        {
            return tex;
        }
        ...
        Texture2D textured3 = new Texture2D(4, 4, TextureFormat.ARGB32, false);
        link.LoadImageIntoTexture(textured3);
        if (mipmap)
        {
            try
            {
                textured3.Resize(num3, num3, TextureFormat.ARGB32, mipmap);
            }
            catch
            {
                textured3.Resize(num3, num3, TextureFormat.ARGB32, false);
            }
        }
        else
        {
...
//     public static Texture2D networkedimage (WWW link)
//     {
